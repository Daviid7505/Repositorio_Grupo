...
public class EmpleadoDaoImplMy8 extends AbstractDaoMy8 implements EmpleadoDao {
	...
	/*
	 * Sobreescribimos el metodo altaEmpleado. A traves de este metodo, se inserta
...
//-----------------------------------------------------------------------------------------------------------------------
	...
	public List<Empleado> buscarEmpleadoPorDepartamento(int idDepartamento) {
		...
		/*
		 * Cramos un nuevo objeto dep y definimos la lista que mostrara los empleados
		 * que pertenecen al departamento pasado por id.
		 */
		DepartamentoDao ddao = new DepartamentoDaoImplMy8();
		List<Empleado> lista = new ArrayList<>();
		...
		try {
			...
			// Establecemos que, aunque el id de departamento no exsita, se muestre la tabla
			// vacia
			if (dep == null) {
				return lista;
			}
			/*
			 * Se ubica el cursor antes de la primera fila y luego recorremos loas filas a
			 * traves de las columnas y se recorren hasta que no haya mas apellidos que
			 * inicien con la subcadena indicada
			 */
			while (rs.next()) {
				Empleado emp = new Empleado();
				PerfilDao pdao = new PerfilDaoImplMy8();
				...
				/*
				 * Establecemos que se agreguen a la lista los empelados que cumplan con la
		...
		/*
		 * Devolvemos la tabla que contiene a todos los empleados que pertenecen al
		 * departamento cuyo id fue establecido en la condicion de búsqueda.
		 */
		return lista;
	}
//-------------------------------------------------------------------------------------------
	...
	public List<Empleado> buscarEmpleadoPorGenero(char genero) {
		...
		/*
		 * Devolvemos la tabla que contiene a todos los empleados que pertenecen al
		 * departamento cuyo id fue establecido en la condición de búsqueda.
		 */
		return lista;
...
//-----------------------------------------------------------------------------------------------------------------------
	...
	/*
	 * Sobreescribimos el metodo buscar por apellido. En este metodo, se ingresa la
	 * subcadena con las letras por las cuales comienza el apellido del empleado y
	 * se obtiene una lista con todos los empleados que contienen dicha subcadena.
	 */
	@Override
	public List<Empleado> buscarEmpleadoPorApellido(String subcadena) {
		...
		try {
			...
			/*
			 * Se ubica el cursor antes de la primera fila y luego recorremos las filas a
			 * traves de las columnas y se recorren hasta que no haya mas apellidos que
			 * inicien con la subcadena indicada
			 */
			while (rs.next()) {
			...
			// En caso de producirse un error, lo imprimimos con la sentencia comprendida en
			// "catch"
		} catch (Exception e) {
			e.printStackTrace();
		}
...
//--------------------------------------------------------------------------------------------
	...
	/*
	 * Sobreescribimos el metodo buscar por pefil. En este metodo, se ingresa el
	 * idPerfil con y se obtiene una lista con todos los empleados que contienen
	 * dicho perfil.
	 */
	@Override
...
//Capturamos las excepciones		
		try {
			...
			/*
			 * Se ubica el cursor antes de la primera fila y luego recorremos loas filas a
			 * traves de las columnas y se recorren hasta que no haya más apellidos que
			 * inicien con la subcadena indicada
			 */
			while (rs.next()) {
				Empleado e = new Empleado();
...
//---------------------------------------------------------------------------------------------	
	...
	/*
	 * Sobreescribimos el metodo salario total. Para este metodo, solo se muetra una
	 * fila que contiene la suma de los salarios de todos los empleados.
	 */
	@Override
	public double salarioTotal() {
		...
		// Capturamos las excepciones en try, definiendo las sentencias sql dentro de
		// este bloque.
		try {
			ps = conn.prepareStatement(sql);
			rs = ps.executeQuery();
...
//------------------------------------------------------------------------------------------------
	...
	/*
	 * Sobreescribimos el metodo salario total por departamentos. Para este metodo,
	 * se muestran las filas con la suma de los salarios de todos los empleados
	 * especificados por departamento.
	 */
	@Override
